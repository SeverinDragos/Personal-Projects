...
Examen& Examen::operator=(const Examen& x) {
  try{
  if (this == &x) throw "S-a incercat atribuirea unui obiect catre el insusi";
...
template <class T>
CatalogIndividual<T>::~CatalogIndividual() { delete []v; }
...
int main()
{
  ...
  Examen* o2;
  cout << " Citirea a n Quiz-uri: 1" << endl;
  cout << " Utilizarea specializarii pentru Quiz: 2" << endl;
  cout << " Exception handling (Consideram exceptie atribuirea unui obiect la el insusi. Throw-ul se face in functia Examen::operator=() ): 3" << endl;
  cout << " Citirea unui vector de obiecte de tip Examen* : 4" << endl;
  cout << " Adaugarea prin operatorul += a unui obiect la vectorul de la punctul 4 : 5" << endl;
  cout << " Iesirea din executie: 0" << endl;
  cout << "dynamic_cast este folosit in specializarea pentru Examen* in cadrul operatorului += si << ." << endl;
  cout << "static_cast este folosit in ierarhia de clase in cadrul operatorilor << si >> ." << endl;
  do {
